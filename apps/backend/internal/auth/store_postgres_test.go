...
func expectSchemaExec(mock sqlmock.Sqlmock) {
	...
	mock.ExpectExec("ALTER TABLE auth_users ADD COLUMN IF NOT EXISTS role").WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("ALTER TABLE auth_users ADD COLUMN IF NOT EXISTS plan").WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("ALTER TABLE auth_users ADD COLUMN IF NOT EXISTS plan_expires_at").WillReturnResult(sqlmock.NewResult(0, 0))
...
func TestPostgresBackend_CreateUser(t *testing.T) {
	...
	user := User{ID: "usr_1", FullName: "User", Email: "user@example.com", PasswordHash: "hash", Role: RoleUser, Plan: PlanFree, CreatedAt: time.Now().UTC(), UpdatedAt: time.Now().UTC()}

	mock.ExpectExec("INSERT INTO auth_users").
		WithArgs(user.ID, user.FullName, user.Email, nil, user.PasswordHash, user.Role, user.Plan, user.PlanExpiresAt, user.CreatedAt, user.UpdatedAt).
...
func TestPostgresBackend_GetUserByEmailByIDAndGoogleSubject(t *testing.T) {
	...
	now := time.Now().UTC()
	rowColumns := []string{"id", "full_name", "email", "avatar_url", "password_hash", "role", "plan", "plan_expires_at", "created_at", "updated_at"}
	queryByEmail := q(`SELECT id, full_name, email, avatar_url, password_hash, role, plan, plan_expires_at, created_at, updated_at FROM auth_users WHERE email = $1`)
	queryByID := q(`SELECT id, full_name, email, avatar_url, password_hash, role, plan, plan_expires_at, created_at, updated_at FROM auth_users WHERE id = $1`)
	queryByGoogleSubject := q(`SELECT u.id, u.full_name, u.email, u.avatar_url, u.password_hash, u.role, u.plan, u.plan_expires_at, u.created_at, u.updated_at
	...
	rows := sqlmock.NewRows(rowColumns).AddRow("usr_1", "User", "user@example.com", nil, "hash", RoleUser, PlanFree, nil, now, now)
	...
	rows = sqlmock.NewRows(rowColumns).AddRow("usr_1", "Renamed User", "user@example.com", "https://avatar.indobang.site/avatars/usr_1/current.webp", "hash", RoleUser, PlanFree, nil, now, updateAt)
	...
	rows = sqlmock.NewRows(rowColumns).AddRow("usr_1", "Renamed User", "user@example.com", "https://avatar.indobang.site/avatars/usr_1/new.webp", "hash", RoleUser, PlanFree, nil, now, updateAt)
	...
	mock.ExpectQuery(updateAvatarURLQuery).WithArgs("usr_1", nil, updateAt).WillReturnRows(sqlmock.NewRows(rowColumns).AddRow("usr_1", "Renamed User", "user@example.com", nil, "hash", RoleUser, PlanFree, nil, now, updateAt))
