...
func integrationTestContext(t *testing.T) (context.Context, context.CancelFunc) {
	t.Helper()
...
func createTempPostgresDatabase(t *testing.T) (string, func()) {
	...
	ctx, cancel := integrationTestContext(t)
	defer cancel()
	...
	dbName := fmt.Sprintf("ytd_http_it_%d_%d", time.Now().UnixNano(), rand.Intn(100000))
	...
	cleanup := func() {
		cleanupCtx, cleanupCancel := integrationTestContext(t)
		defer cleanupCancel()
