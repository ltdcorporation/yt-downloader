...
func TestValidateTikTokURL(t *testing.T) {
	valid := []string{
		"https://www.tiktok.com/@user/video/12345",
		"https://m.tiktok.com/@user/video/12345?is_copy_url=1",
	...
	for _, input := range valid {
		if err := validateTikTokURL(input); err != nil {
	...
	tests := []struct {
		...
		},
		{
			...
			url:  "ftp://www.tiktok.com/@user/video/1",
			err:  "URL must start with http or https",
		},
		{
			name: "invalid host",
			url:  "https://example.com/@user/video/1",
			err:  "URL must be a TikTok link",
		},
		{
			name: "unsupported path",
			url:  "https://www.tiktok.com/@user/live",
			err:  "unsupported TikTok URL path",
		},
		{
...
func TestResolve_RotatesToCookieProfile(t *testing.T) {
	...
	script := makeFakeYTDLPScript(t, fakeYTDLPScriptOptions{
		...
		ExpectedURL:    "https://www.tiktok.com/@user/video/12345",
	...
	result, err := resolver.Resolve(context.Background(), "https://www.tiktok.com/@user/video/12345")
	if err != nil {
		t.Fatalf("expected resolve success, got err: %v", err)
	}
	if result.Title != "TikTok Test Video" {
...
func TestResolve_NoProfilesConfigured(t *testing.T) {
	...
	_, err := resolver.Resolve(context.Background(), "https://www.tiktok.com/@user/video/12345")
...
func TestResolve_PublicModeSuccess(t *testing.T) {
	...
	script := makeFakeYTDLPScript(t, fakeYTDLPScriptOptions{
		Stdout:      mustJSON(t, payload),
		ExpectedURL: "https://www.tiktok.com/@user/video/987654321",
	})
	...
	result, err := resolver.Resolve(context.Background(), "https://www.tiktok.com/@user/video/987654321")
	if err != nil {
		t.Fatalf("expected resolve success, got err: %v", err)
	}
...
func TestResolve_WithCurlInput(t *testing.T) {
	...
	script := makeFakeYTDLPScript(t, fakeYTDLPScriptOptions{
		Stdout:      mustJSON(t, payload),
		ExpectedURL: "https://www.tiktok.com/@user/video/1111",
	})
	...
	input := `curl "https://www.tiktok.com/@user/video/1111" -H "Referer: https://www.tiktok.com/" -A "Mozilla/5.0 Test"`
...
func TestNewResolver_DefaultAndExplicitQuality(t *testing.T) {
	fallback := NewResolver("yt-dlp", "  node  ", 0, 777, "  /tmp/tt-cookies  ", "  /tmp/a.txt,/tmp/b.txt  ", true)
	...
	if fallback.cookiesDir != "/tmp/tt-cookies" {
...
func TestResolve_EarlyValidationErrors(t *testing.T) {
	t.Run("requires ytdlp binary", func(t *testing.T) {
		...
		_, err := resolver.Resolve(context.Background(), "https://www.tiktok.com/@user/video/1")
	...
	t.Run("parse input error", func(t *testing.T) {
		...
		_, err := resolver.Resolve(context.Background(), `curl "https://www.tiktok.com/@user/video/1`)
	...
	t.Run("invalid host", func(t *testing.T) {
		resolver := NewResolver("yt-dlp", "", 1080, 0, "", "", true)
		_, err := resolver.Resolve(context.Background(), "https://example.com/@user/video/1")
		if err == nil || !strings.Contains(err.Error(), "URL must be a TikTok link") {
			t.Fatalf("expected tiktok host validation error, got %v", err)
		}
	})
}
...
func TestResolve_FailureWrappingByCandidateCount(t *testing.T) {
	t.Run("single candidate returns direct error", func(t *testing.T) {
		...
		_, err := resolver.Resolve(context.Background(), "https://www.tiktok.com/@user/video/123")
		if err == nil {
		...
		if strings.Contains(err.Error(), "failed to resolve TikTok URL with") {
	...
	t.Run("multiple candidates returns aggregate error", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "failed to resolve TikTok URL with 2 cookie profiles") {
	...
	t.Run("non-retryable typed error bypasses aggregate wrapping", func(t *testing.T) {
		...
		if resolveErr.Code != ErrCodeTTHLSOnlyNotSupported {
			t.Fatalf("expected code %q, got %q", ErrCodeTTHLSOnlyNotSupported, resolveErr.Code)
		}
	})
}
...
func TestResolve_ResponseEdgeCases(t *testing.T) {
	...
	t.Run("accepts direct mp4 when codec metadata is empty", func(t *testing.T) {
		payload := ytdlpOutput{
			Title: "Direct MP4",
			Formats: []ytdlpFormat{
				{FormatID: "direct-256", Ext: "mp4", VideoCodec: "", AudioCodec: "", Protocol: "https", URL: "https://video-cdn.example/270.mp4", Height: 270, Filesize: 1000},
				{FormatID: "direct-832", Ext: "mp4", VideoCodec: "", AudioCodec: "", Protocol: "https", URL: "https://video-cdn.example/360.mp4", Height: 360, Filesize: 2000},
				{FormatID: "hls-250", Ext: "mp4", VideoCodec: "avc1", AudioCodec: "none", Protocol: "m3u8_native", URL: "https://video-cdn.example/360.m3u8", Height: 360},
		...
		result, err := resolver.Resolve(context.Background(), "https://www.tiktok.com/@user/video/1")
		if err != nil {
			t.Fatalf("expected resolve success, got %v", err)
		}
		if len(result.Formats) != 2 {
			t.Fatalf("expected 2 formats from direct mp4 fallback, got %d (%#v)", len(result.Formats), result.Formats)
		}
		if result.Formats[0].ID != "direct-256" || result.Formats[1].ID != "direct-832" {
...
func TestSelectFormats_AcceptsTikTokDirectMP4Fallback(t *testing.T) {
	resolver := NewResolver("yt-dlp", "", 1080, 0, "", "", true)

	formats := resolver.selectFormats([]ytdlpFormat{
		{FormatID: "direct-256", Ext: "mp4", URL: "https://video-cdn.example/270.mp4", Height: 270, Protocol: "https", Filesize: 1000},
		{FormatID: "direct-832", Ext: "mp4", URL: "https://video-cdn.example/360.mp4", Height: 360, Protocol: "https", Filesize: 2000},
	...
	if len(formats) != 2 {
		t.Fatalf("expected 2 direct mp4 formats, got %d (%#v)", len(formats), formats)
	}
	if formats[0].ID != "direct-256" || formats[1].ID != "direct-832" {
		t.Fatalf("unexpected format IDs: %#v", formats)
	}
}

func TestIsLikelyTikTokDirectMP4(t *testing.T) {
	tests := []struct {
		name string
		in   ytdlpFormat
		want bool
		...
		{
			name: "valid direct mp4",
			in: ytdlpFormat{
				FormatID: "direct-832",
				Ext:      "mp4",
				Protocol: "https",
				Height:   360,
				URL:      "https://tiktok.cdn/video.mp4",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			in: ytdlpFormat{
				...
				Protocol:   "https",
				Height:     360,
				URL:        "https://tiktok.cdn/video-only.mp4",
			},
			want: false,
		},
		{
			...
			in: ytdlpFormat{
				...
				Protocol:   "https",
				Height:     360,
				URL:        "https://tiktok.cdn/audio.mp4",
				FormatNote: "Audio",
			},
			want: false,
		},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := isLikelyTikTokDirectMP4(tc.in)
			if got != tc.want {
...
func TestIsLikelyHLSOnlySource(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			raw: []ytdlpFormat{
				{FormatID: "hls-250", Protocol: "m3u8_native", VideoCodec: "avc1", AudioCodec: "none", Height: 360},
				{FormatID: "direct-832", Protocol: "https", Height: 360, URL: "https://video-cdn.example/360.mp4"},
			},
			want: false,
		},
		{
