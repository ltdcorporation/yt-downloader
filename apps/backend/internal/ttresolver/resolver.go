...
const (
	ErrCodeTTHLSOnlyNotSupported = "tt_hls_only_not_supported"
)
...
func (r *Resolver) Resolve(ctx context.Context, rawURL string) (ResolveResult, error) {
	...
	if err := validateTikTokURL(targetURL); err != nil {
		return ResolveResult{}, err
	}

	candidates := r.buildCookieCandidates()
	if len(candidates) == 0 {
		return ResolveResult{}, errors.New("tiktok resolver has no cookie profile configured")
	}
	...
	return ResolveResult{}, fmt.Errorf("failed to resolve TikTok URL with %d cookie profiles: %v", len(candidates), lastErr)
}
	...
	if len(formats) == 0 {
		if isLikelyHLSOnlySource(payload.Formats) {
			return ResolveResult{}, &ResolveError{
				Code:    ErrCodeTTHLSOnlyNotSupported,
				Message: "TikTok video is HLS-only and not supported yet",
			}
		}
...
func validateTikTokURL(rawURL string) error {
	...
	case "vm.tiktok.com", "vt.tiktok.com":
		...
		return errors.New("unsupported TikTok URL path")
	case "tiktok.com", "www.tiktok.com", "m.tiktok.com":
		if len(parts) >= 3 && strings.HasPrefix(parts[0], "@") && parts[1] == "video" && parts[2] != "" {
			return nil
		}
		if len(parts) >= 2 && parts[0] == "t" && parts[1] != "" {
			return nil
		}
		if len(parts) >= 2 && parts[0] == "embed" && parts[1] != "" {
			return nil
		}
		return errors.New("unsupported TikTok URL path")
	default:
		return errors.New("URL must be a TikTok link")
	}
}
...
func isNonRetryableResolveErr(err error) bool {
	...
	return resolveErr.Code == ErrCodeTTHLSOnlyNotSupported
}
...
func (r *Resolver) selectFormats(raw []ytdlpFormat) []Format {
	...
	for _, item := range raw {
		if !isProgressiveMP4(item) && !isLikelyTikTokDirectMP4(item) {
			continue
		}
...
func isLikelyTikTokDirectMP4(item ytdlpFormat) bool {
	if strings.ToLower(item.Ext) != "mp4" {
		return false
	}
