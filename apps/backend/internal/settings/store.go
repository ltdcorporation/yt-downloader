...
type backend interface {
	...
	GetOrCreateSnapshot(ctx context.Context, userID string, now time.Time) (Snapshot, error)
...
func NewStore(cfg config.Config, logger *log.Logger) *Store {
	if strings.TrimSpace(cfg.PostgresDSN) != "" {
		if logger != nil {
			logger.Printf("settings store engine=postgres")
		}
		return &Store{backend: newPostgresBackend(cfg.PostgresDSN)}
	}

	if logger != nil {
		logger.Printf("settings store engine=memory (POSTGRES_DSN empty)")
	}
...
func (s *Store) GetOrCreateSnapshot(ctx context.Context, userID string, now time.Time) (Snapshot, error) {
	if s == nil || s.backend == nil {
		return Snapshot{}, errors.New("settings store is not initialized")
	}
	...
	snapshot, err := s.backend.GetOrCreateSnapshot(ctx, trimmedUserID, now.UTC())
	if err != nil {
		return Snapshot{}, err
	}
...
func (s *Store) ApplyPatch(ctx context.Context, params ApplyPatchParams) (Snapshot, error) {
	...
	if before.UserID != after.UserID {
		return Snapshot{}, &ValidationError{Message: "before/after user_id mismatch"}
	}
	...
	if params.ActorUserID == "" {
		params.ActorUserID = before.UserID
	}
...
func DefaultSnapshot(userID string, now time.Time) Snapshot {
	if now.IsZero() {
		now = time.Now().UTC()
	}
